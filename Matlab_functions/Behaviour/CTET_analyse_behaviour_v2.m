...
for nF=1:length(files)
    ...
    behav_Miss=nan(length(targets_idx),1);
    ...
    %%% Look for FAs
    for nR=1:length(resp_idx)
        dist_to_resp=(all_idx(find(ismember(all_idx,resp_idx(nR)))+(-10:0))-all_idx(find(ismember(all_idx,resp_idx(nR)))))/hdr.Fs;
        if isempty(find(all_val(find(ismember(all_idx,resp_idx(nR)))+(-10:0))==max(unique_values) & dist_to_resp>-5.2))
            this_FAs=[this_FAs ; resp_idx(nR)];
        end
    end
    
    res_mat=[res_mat; [SubN*ones(size(behav_Miss,1),1) SessN*ones(size(behav_Miss,1),1) behav_Miss behav_RT behav_Impulse (1:size(behav_Miss,1))']];
    drug_cond=[drug_cond ; repmat({DrugC},size(behav_Miss,1),1)];
end

%%
res_mat2=res_mat(~isnan(res_mat(:,3)),:);
table=array2table(res_mat2,'VariableNames',{'SubID','SessN','Miss','RT','Impulse','TrialN'});
