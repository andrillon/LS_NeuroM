...
all_peaks=[];
    ...
    all_fit=[];
        ...
        for nBl=1:10
            ...
            nwindows=floor((last_trial-first_trial)/hdr.Fs/10);
            for nEl=1:64
                fprintf('\b\b\b\b\b\b%2.0f-%2.0f\n',nBl,nEl)
                temp_pow=[];
                for nW=1:2*nwindows-1
                    %        block_data=temp_data(:,first_trial:(first_trial+235520-1));
                    block_data=temp_data(nEl,(first_trial+(nW-1)*5*hdr.Fs):(first_trial+(nW-1)*5*hdr.Fs+10*hdr.Fs));
                    
                    [faxis, pow] = get_PowerSpec(block_data, hdr.Fs, 0 ,0);
                    %                     pow = interp1(faxis,pow,myfreqs);
                ...
                try
                    fooof_results = fooof(faxis, mean(temp_pow), f_range, settings,1);
                    
                    myfreqs=fooof_results.freqs;
                    
                    all_pow(nBl,nEl,:)=mean(temp_pow);
