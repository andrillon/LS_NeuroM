...
for nD=1:3
    ...
    for nE=1:length(layout.label)-2
        ...
        for nS=1:length(Subs)
            ...
            avY1=[avY1 ; grpstats(Y1{1,nSc},X{1,nSc})];
            avY2=[avY2 ; grpstats(Y2{1,nSc},X{1,nSc})];
            avY3=[avY3 ; grpstats(Y3{1,nSc},X{1,nSc})];
            avX=[avX ; grpstats(X{1,nSc},X{1,nSc})];
            avM=[avM ; grpstats(M{1,nSc},X{1,nSc})];
            %         end
            %     end
        end
        [paths, stats2] = mediation(X, Y1, M, 'verbose', 'bootsamples', 10000);
        ...
        [paths, stats2] = mediation(X, Y2, M, 'verbose', 'bootsamples', 10000);
        ...
        [paths, stats2] = mediation(X, Y3, M, 'verbose', 'bootsamples', 10000);
...
for nD=1:3
    ...
    subplot(2,3,4)
    temp=all_stats{nD,1}{2}(:,5);
    tempPv=all_stats{nD,1}{3}(:,5);
    temp(tempPv>0.05)=0;
%     temp(tempPv>fdr(all_Pvs,0.05))=0;
    ...
    subplot(2,3,5)
    temp=all_stats{nD,2}{2}(:,5);
    tempPv=all_stats{nD,2}{3}(:,5);
    temp(tempPv>0.05)=0;
%     temp(tempPv>fdr(all_Pvs,0.05))=0;
    simpleTopoPlot_ft(temp, layout,'on',[],0,1);
    caxis([-1 1]*5)
    ...
    subplot(2,3,6)
    temp=all_stats{nD,3}{2}(:,5);
    tempPv=all_stats{nD,3}{3}(:,5);
    temp(tempPv>0.05)=0;
%     temp(tempPv>fdr(all_Pvs,0.05))=0;
    simpleTopoPlot_ft(temp, layout,'on',[],0,1);
    caxis([-1 1]*5)
