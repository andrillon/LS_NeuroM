...
if size(model_est{2},2)==6
    for nDrug=1:3
        ...
        for nsign=1:2
            if nsign==1
                sig_elec=model_est{1}(model_est{1}(:,4)<clus_alpha & model_est{1}(:,3)>0 & model_est{1}(:,5)==nDrug,1);
                tstat_elec=model_est{1}(model_est{1}(:,4)<clus_alpha & model_est{1}(:,3)>0 & model_est{1}(:,5)==nDrug,3);
            else
                sig_elec=model_est{1}(model_est{1}(:,4)<clus_alpha & model_est{1}(:,3)<0 & model_est{1}(:,5)==nDrug,1);
                tstat_elec=model_est{1}(model_est{1}(:,4)<clus_alpha & model_est{1}(:,3)<0 & model_est{1}(:,5)==nDrug,3);
            end
