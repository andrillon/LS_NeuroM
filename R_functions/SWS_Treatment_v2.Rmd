...
ggplot(aggdata_SW, aes(x = Elec, y = SWdens, colour = Treatment)) + geom_line() + geom_point() + scale_colour_brewer(palette = "Dark2") + scale_fill_brewer(palette = "Dark2") + ggtitle("Slow wave density according to electrodes and treatment") + ylab("Slow Wave Density") + xlab("Electrode") + xlim(unique(aggdata_SW$Elec))
