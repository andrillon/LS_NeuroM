...
SWS_data <- read.table("../Tables/CTET_SWdetection_thr90_allE_P2P_avDens_behav_vec_full_v6.txt", header = TRUE, sep = ",", stringsAsFactors = FALSE)
...
- sig lm.pp1 (*) and lm.pp2 (***) - blockN and Treatment have an effect on P2P
...
```
- According to AIC and BIC lm.pp2 is the winning model
-Showed different results; 
  -for the non-interaction model (lm.pp2), block number and all the treatments had a significant effect (p <.001) EXCEPT CIT (Different from prev. analysis which showed that it was MPH that did not have a sig. effect)
  **NOTE: lm.pp3 was the winning model in the previous analysis according to BIC, but here it is not
...
```
  ...
  - For the non-interaction model (lm.ps2), block number and all the treatments had a significant effect (p <.001) EXCEPT CIT (same as prev. analysis)
  -model failed to converge for lm.ps2 in previous analysis but was fine for this analysis 
...
ggplot(data=dens.graph, aes(x=Treatment, y=SWdens, fill = Treatment)) +
  geom_bar(stat="identity") + geom_errorbar(aes(ymin = SWdens-se, ymax = SWdens+se), width = .5, size = .6) + scale_fill_discrete( name = "Treatment", breaks = c("PLA", "MPH", "ATM", "CIT"), labels = c("Placebo", "Methylphenidate", "Atomoxetine", "Citalopram")) + ylab("SW density")+ scale_fill_manual(values = c("#666666", "#d95f02", "#7570b3", "#1b9e77")) + coord_cartesian(ylim =c(4, 6)) + scale_y_continuous(labels = scales::number_format(accuracy = .01)) + theme_classic()+theme(legend.position = "none",axis.title=element_text(size=14),axis.text=element_text(size=14), axis.title.x = element_blank()) 

ggsave(file="../Figures/CTET_BarPlot_Dens_2021.png", dpi = 600, 
       width = 15/2,
      height = 18/2,
    units = c("cm"))
```
...
ggsave(file="../Figures/CTET_LinePlot_P2P_2021.png", 
       width = 30/2,
      height = 18/2,
    units = c("cm"))
...
ggplot(data=p2p2.graph, aes(x=Treatment, y=P2P, fill = Treatment)) +
  geom_bar(stat="identity") + geom_errorbar(aes(ymin = P2P-se, ymax = P2P+se), width = .5, size = .6) + scale_fill_discrete( name = "Treatment", breaks = c("PLA", "MPH", "ATM", "CIT"), labels = c("Placebo", "Methylphenidate", "Atomoxetine", "Citalopram")) + ylab("Mean Peak to Peak")+ scale_fill_manual(values = c("#666666", "#d95f02", "#7570b3", "#1b9e77")) + coord_cartesian(ylim =c(35, 43)) + scale_y_continuous(labels = scales::number_format(accuracy = .01)) + theme_classic()+theme(legend.position = "none",axis.title=element_text(size=14),axis.text=element_text(size=14), axis.title.x = element_blank()) 

ggsave(file="../Figures/CTET_BarPlot_P2P_2021.png", dpi = 600, 
       width = 15/2,
      height = 18/2,
    units = c("cm"))
```
...
ggsave(file="../Figures/CTET_LinePlot_PosSlope_2021.png", 
       width = 30/2,
      height = 18/2,
    units = c("cm"))
...
ggplot(data=ps2.graph, aes(x=Treatment, y=PosSlope, fill = Treatment)) +
  geom_bar(stat="identity") + geom_errorbar(aes(ymin = PosSlope-se, ymax = PosSlope+se), width = .5, size = .6) + scale_fill_discrete( name = "Treatment", breaks = c("PLA", "MPH", "ATM", "CIT"), labels = c("Placebo", "Methylphenidate", "Atomoxetine", "Citalopram")) + ylab("Mean Positive Slope")+ scale_fill_manual(values = c("#666666", "#d95f02", "#7570b3", "#1b9e77")) + coord_cartesian(ylim =c(350, 500)) + scale_y_continuous(labels = scales::number_format(accuracy = .01)) + theme_classic()+theme(legend.position = "none",axis.title=element_text(size=14),axis.text=element_text(size=14), axis.title.x = element_blank())

ggsave(file="../Figures/CTET_BarPlot_PosSlope_2021.png", dpi = 600, 
       width = 15/2,
      height = 18/2,
    units = c("cm"))
```
...
ggsave(file="../Figures/CTET_LinePlot_NegSlope_2021.png", 
       width = 30/2,
      height = 18/2,
    units = c("cm"))
...
ggsave(file="../Figures/CTET_BarPlot_NegSlope_2021.png", dpi = 600, 
       width = 15/2,
      height = 18/2,
    units = c("cm"))
```