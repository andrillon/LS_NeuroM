...
##Barplot 
dens.graph <- summarySE(SWS_data1, measurevar = "SWdens", groupvars = c( "Treatment","SubID"))
dens.graph$Treatment<-ordered(dens.graph$Treatment,c("PLA","MPH","ATM","CIT"))

ggplot(dens.graph, aes(x = Treatment, y = SWdens, fill = Treatment)) +
  geom_flat_violin(aes(fill = Treatment), position = position_nudge(x = .1, y = 0), adjust = 1.5, trim = FALSE, alpha = .25, colour = NA) + ylim(2, 12) +
  geom_point(aes(x = as.numeric(Treatment)-.225, y = SWdens, colour = Treatment), position = position_jitter(width = .05), size = 4, shape = 20) + 
  geom_boxplot(aes(x = Treatment, y = SWdens, fill = Treatment), outlier.shape = NA, alpha = .5, width = .225, colour = "black") +
  scale_fill_manual(values = c("#666666", "#d95f02", "#7570b3", "#1b9e77")) + scale_colour_manual(values = c("#666666", "#d95f02", "#7570b3", "#1b9e77")) +xlab("Treatment") + ylab("SW Dens") + theme(panel.background = element_blank(), axis.line = element_line(colour = "black"))
