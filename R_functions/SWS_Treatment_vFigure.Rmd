...
lm.sw1 <- lmer(SWdens  ~ 1  + Treatment + (1|SubID), data = SWS_data1, REML = FALSE)
...
##Effects of block number on peak to peak 
lm.pp1 <- lmer(P2P  ~ 1  + Treatment + (1|SubID), data = SWS_data2, REML = FALSE)
...
summary_SWdens
summary_SWdens$Treatment<-ordered(summary_SWdens$Treatment,c("PLA","MPH","ATM","CIT"))
...
ggplot(summary_SWdens, aes(x = Treatment, y = SWdens, fill = Treatment)) +
  ...
  scale_fill_manual(values = c("#666666", "#d95f02", "#7570b3", "#1b9e77")) + scale_colour_manual(values = c("#666666", "#d95f02", "#7570b3", "#1b9e77")) +xlab("Treatment") + ylab("Slow Wave Density (wave/min)") + theme(panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  font("ylab", size = 12,family="Arial", face="bold") + font("xlab", size = 12,family="Arial", face="bold") + font("y.text", size = 12,family="Arial", face="bold") + font("x.text", size = 12,family="Arial", face="bold") +
  scale_y_continuous(labels = scales::number_format(accuracy = .1)) + theme(legend.position="none") + coord_cartesian(ylim =c(0,12))

ggsave(file="../Figures/CTET_RainCloudPlot_SWdens.png", 
       width = 30/4,
      height = 18/2,
    units = c("cm"))  
```
