...
### Neg MPH on RT
med.fit1 <- glmer(C_Neg_MPH  ~ 1 + Treatment + (1|SubID), data = SWS_data3)
out.fit1 <- glmer(Hit_RT  ~ 1 + C_Neg_MPH * Treatment + (1|SubID), data = SWS_data3)
med.out1 <- mediate(med.fit1, out.fit1, treat = "Treatment", mediator = "C_Neg_MPH",boot = FALSE, sims = nsims)
print(' MPH - Hit RT (Negative MPH cluster)')
summary(med.out1)

## Neg MPH on misses 
med.fit1 <- glmer(C_Neg_MPH  ~ 1 + Treatment + (1|SubID), data = SWS_data3)
out.fit1 <- glmer(Miss  ~ 1 + C_Neg_MPH * Treatment + (1|SubID), data = SWS_data3)
...
## Neg MPH on FA
med.fit1 <- glmer(C_Neg_MPH  ~ 1 + Treatment + (1|SubID), data = SWS_data3)
out.fit1 <- glmer(FA  ~ 1 + C_Neg_MPH * Treatment + (1|SubID), data = SWS_data3)
...
NEG MPH ON Misses: 
  ...
  -non-significant mediated path 
NEG MPH ON FA: 
  - mediated path for control (p = .0238) and treated (p <.01) are significant
...
### Neg ATM on RT
med.fit1 <- glmer(C_Neg_ATM  ~ 1 + Treatment + (1|SubID), data = SWS_data4)
out.fit1 <- glmer(Hit_RT  ~ 1 + C_Neg_ATM * Treatment + (1|SubID), data = SWS_data4)
...
## Neg ATM on misses 
med.fit1 <- glmer(C_Neg_ATM  ~ 1 + Treatment + (1|SubID), data = SWS_data4)
out.fit1 <- glmer(Miss  ~ 1 + C_Neg_ATM * Treatment + (1|SubID), data = SWS_data4)
...
## Neg ATM on FA
med.fit1 <- glmer(C_Neg_ATM  ~ 1 + Treatment + (1|SubID), data = SWS_data4)
out.fit1 <- glmer(FA  ~ 1 + C_Neg_ATM * Treatment + (1|SubID), data = SWS_data4)
...
summary(med.out6)
#fit<-mglmer(d=SWS_data4,id="SubID",x="Treatment",m="C_Neg_ATM",y="Miss",iter=1000,core=2)
```
NEG ATM ON RT: 
  - significant direct effect for control (p = .013) and treated (p = .024) and total effect (p = .017)
  - non-significant mediated path 
  - ADE average was significant (p = .014)
...
```{r}
### Pos ATM on RT
med.fit1 <- glmer(C_Pos_ATM  ~ 1 + Treatment + (1|SubID), data = SWS_data4)
out.fit1 <- glmer(Hit_RT  ~ 1 + C_Pos_ATM * Treatment + (1|SubID), data = SWS_data4)
...
## Pos ATM on misses 
med.fit1 <- glmer(C_Pos_ATM  ~ 1 + Treatment + (1|SubID), data = SWS_data4)
out.fit1 <- glmer(Miss  ~ 1 + C_Pos_ATM * Treatment + (1|SubID), data = SWS_data4)
...
## Pos ATM on FA
med.fit1 <- glmer(C_Pos_ATM  ~ 1 + Treatment + (1|SubID), data = SWS_data4)
out.fit1 <- glmer(FA  ~ 1 + C_Pos_ATM * Treatment + (1|SubID), data = SWS_data4)
...
POS ATM ON RT: 
  - Significant direct path for control (p = .025) and treated (p = .023) and total effect (p = .02)
  - non-significant mediated path 
  -Significant ADE average (p = .022)
  
POS ATM ON Misses: 
  -Non-significant mediated path and total effect
  -Significant ACME (Average) effect (p = .046)
...
### Pos CIT on RT
med.fit1 <- glmer(C_Pos_CIT  ~ 1 + Treatment + (1|SubID), data = SWS_data5)
out.fit1 <- glmer(Hit_RT  ~ 1 + C_Pos_CIT * Treatment + (1|SubID), data = SWS_data5)
...
## Pos CIT on misses 
med.fit1 <- glmer(C_Pos_CIT  ~ 1 + Treatment + (1|SubID), data = SWS_data5)
out.fit1 <- glmer(Miss  ~ 1 + C_Pos_CIT * Treatment + (1|SubID), data = SWS_data5)
...
## Pos CIT on FA
med.fit1 <- glmer(C_Pos_CIT  ~ 1 + Treatment + (1|SubID), data = SWS_data5)
out.fit1 <- glmer(FA  ~ 1 + C_Pos_CIT * Treatment + (1|SubID), data = SWS_data5)
...
```
POS CIT ON RT: 
  - Significant mediated path for treated (p < .01) and total effect (p = .029)
  -Non-significant direct path 
  -significant prop. mediated effect for treated (p = .0374)
  -significant ACME average (p = .0344)
  
POS CIT ON Misses: 
  -significant mediated path for control (p < .001) and treated (p < .01)
