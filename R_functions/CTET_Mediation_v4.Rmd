...
SWS_data <- read.table("../Tables/CTET_SWdetection_thr90_allE_P2P_avDens_behav_vec_full_v3_CLUSTERS.txt", header = TRUE, sep = ",", stringsAsFactors = FALSE)
...
### Neg MPH on SD RT
med.fit1 <- lmer(C_Neg_MPH  ~ 1 + Treatment + (1|SubID), data = SWS_data3)
out.fit1 <- lmer(STD_RT  ~ 1 + C_Neg_MPH * Treatment + (1|SubID), data = SWS_data3)
med.out13 <- mediate(med.fit1, out.fit1, treat = "Treatment", mediator = "C_Neg_MPH",boot = FALSE, sims = nsims)
print(' MPH - SD RT (Negative MPH cluster)')
summary(med.out13)
...
### Neg ATM on SD RT
med.fit1 <- lmer(C_Neg_ATM  ~ 1 + Treatment + (1|SubID), data = SWS_data4)
out.fit1 <- lmer(STD_RT  ~ 1 + C_Neg_ATM * Treatment + (1|SubID), data = SWS_data4)
med.out14 <- mediate(med.fit1, out.fit1, treat = "Treatment", mediator = "C_Neg_ATM",robustSE = TRUE, sims = nsims)
print(' ATM - SD RT (Negative ATM cluster)')
summary(med.out14)
...
### Pos ATM on SD RT
med.fit1 <- lmer(C_Pos_ATM  ~ 1 + Treatment + (1|SubID), data = SWS_data4)
out.fit1 <- lmer(STD_RT  ~ 1 + C_Pos_ATM * Treatment + (1|SubID), data = SWS_data4)
med.out15 <- mediate(med.fit1, out.fit1, treat = "Treatment", mediator = "C_Pos_ATM",robustSE = TRUE, sims = nsims)
print(' ATM - SD RT (Positive ATM cluster)')
summary(med.out15)
...
### Pos CIT on SD RT
med.fit1 <- lmer(C_Pos_CIT  ~ 1 + Treatment + (1|SubID), data = SWS_data5)
out.fit1 <- lmer(STD_RT  ~ 1 + C_Pos_CIT * Treatment + (1|SubID), data = SWS_data5)
med.out16 <- mediate(med.fit1, out.fit1, treat = "Treatment", mediator = "C_Pos_CIT",robustSE = TRUE, sims = nsims)
print(' CIT - Hit SD RT (Positive CIT cluster)')
summary(med.out16)
```
POS CIT ON RT: 
